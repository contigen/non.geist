...
- TypeScript `.d.ts` type declarations for all CSS exports and subpath exports to support side-effect imports under `moduleResolution: "bundler"`, `"node16"`, and `"nodenext"` without TypeScript errors (`TS2882`).
- Generator support in `scripts/` to automatically produce `.d.ts` files alongside generated `.css` font-face files.
